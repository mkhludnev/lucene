 ...
 * its block semantics that list is indexed atomically, with no flush allowed to land in the
 * middle of it, so -- exactly like {@link AIJoinColumnWriter}'s single {@code addBatch} -- the
 * whole batch is guaranteed to end up doc-for-doc (list index == doc id) in one sidecar segment,
 * keeping doc 0's edges and every from-doc id aligned the same way.
 */
final class AIJoinDocWriter extends AIJoinWriter {
   ...
   * to-side doc ids, spread across the batch's docs, and the edges companion fields, always
   * added to doc 0 even when the pair maps nothing, so a once-built pair is detectable in the
   * join index and never rebuilt.
   */
  private static void addJoinColumns(List<Document> docs, JoinColumnModel mapping, String pairFieldName)
      throws IOException {
    ...
    addEdges(docs, AIJoinUtil.TO_COUNT_PREFIX + pairFieldName, new int[] {mapping.edges().toCount()});
  }

  /**
   * Adds a pair's {min, max} (or count) values to doc 0, mirroring {@link
   * AIJoinColumnWriter}'s {@code edgesColumn}, which puts both values at doc 0 too.
   */
  ...
  /**
   * Adds the doc-map field: batch-local doc number is the from-side doc id and the
   * SORTED_NUMERIC docvalue is the matching to-side doc id. From docs without a match get no
   * value, hence the field is sparse.
   */
